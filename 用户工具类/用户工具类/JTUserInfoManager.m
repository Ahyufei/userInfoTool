...
static JTUserInfoManager *userInfo;
@implementation JTUserInfoManager
...
+ (instancetype)shared{
    ...
    dispatch_once(&once, ^{
        userInfo = [[JTUserInfoManager alloc]init];
    });
    return userInfo;
}
#pragma mark-保存数据
+ (void)saveInfo:(NSDictionary *)infoDict {
    ...
    for (NSString *key in allKeys) {
        ...
        JTUserInfoManager *manager = [JTUserInfoManager shared];
...
+ (void)cleanLocalInfo{
    ...
    for (NSString *attribute in allAttribute) {
        [[NSUserDefaults standardUserDefaults] setObject:@"" forKey:attribute];
    }
}
